    ...
    try:
        ...
        if drafter.get_player_bans()[ctx.author.display_name] < drafter.ban_limit:
            ...
            if drafter.ban_limit < 2:
                ...
                await ctx.send(
                    f"**{ctx.author.display_name}**, you've already banned {drafter.ban_limit} civilizations!")
    except KeyError:
        await ctx.send(
...
@bot.command()
async def draft(ctx):
    drafter_copy = copy.deepcopy(drafter)
    msg = drafter.draft()
    try:
        await ctx.send(msg)
...
async def reroll(ctx):
    drafter = copy.deepcopy(drafter_copy)
    msg = drafter.draft()
    try:
        await ctx.send(msg)
    except HTTPException:
